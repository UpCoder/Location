...
'''
    ...
    结果：？
'''
def extract_features_version2(path, mail_id, shop_mail_txt_path):
    '''
    :param path: 测试文件的路径
    :param mail_id: 当前预测的mail的ID
    :return: 训练集合的feature，label 测试集合的feature, 以及当前商场对应的shop 的ID（主要是用来写入csv文件）
    ...
    wifi_strength_dict = prepare.Tools.load_dict(shop_mail_txt_path)
    print 'the number of wifi source is ', len(wifi_dict)
    ...
    all_features = np.concatenate(
        [do_pca_one_feature(all_features_show), do_pca_one_feature(all_features_strength)],
        axis=1
    )
    training_labels = np.array(training_labels)
...
def generate_predict(data_dir, re_name, output_path):
    ...
    for path_index, path in enumerate(paths):
        ...
        training_features, training_labels, testing_features, shop_ids = extract_features_version2(
            path,
            mail_id,
            shop_mail_txt_path=os.path.join(prepare.data_dir, 'wifi_strength.txt')
        )
